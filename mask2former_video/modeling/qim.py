...
class QueryInteractionModule(QueryInteractionBase):
    def __init__(self, dim_in, hidden_dim, dim_out):
        self.random_drop = 0.1 #args.random_drop
        self.fp_ratio = 0.3 #args.fp_ratio
        self.update_query_pos = True #args.update_query_pos
        self.merger_dropout = 0 #args.merger_dropout
        super().__init__(dim_in, hidden_dim, dim_out)

    def _build_layers(self, dim_in, hidden_dim, dim_out):
        dropout = self.merger_dropout
    ...
    def _select_active_tracks(self, data: dict) -> Instances:
        track_instances: Instances = data
        if self.training:
    ...
    def forward(self, track_instances, init_track_instances) -> Instances:
